...
ozpolls <- ozpolls |> 
  mutate(firm = if_else(firm == "YouGov/Galaxy", "Galaxy", firm),
         firm = if_else(firm == "YouGov-Galaxy", "Galaxy", firm)) |> 
  filter(end_date < as.Date("2019-05-18"))
...
all_firms_index <- ozpolls |> 
  filter(!firm %in% c("Election result") & firm %in% firms_today) |> 
  select(firm) |> 
  distinct() |> 
...
st <- system.time({
  model_2pp <- stan(file = 'model-2pp/model-2pp.stan', 
                    data = model_data, 
                    chains = 4, 
